...
func calculate_king_move(cell: Vector2) -> Array:
	var moves = []
	var king_team = board.board_state[cell.x][cell.y].team
	...
	for p in possible:
		...
		if grid.is_within_bounds(new_cell):
			...
			elif board.board_state[new_cell.x][new_cell.y].team != king_team:
				moves.append(new_cell)
	var legal_moves = []
	for m in moves:
	...
	for p in possible:
		for i in range(1,9):
			...
			if grid.is_within_bounds(new_cell):
				if board.board_state[new_cell.x][new_cell.y] == null:
					moves.append(new_cell)
				elif board.board_state[new_cell.x][new_cell.y] != null and board.board_state[new_cell.x][new_cell.y].team != board.board_state[cell.x][cell.y].team:
					moves.append(new_cell)
					break
				else:
					break
